...
export async function deferInteraction(
  ...
  try {
    ...
    ctx.logger.error("Deferred interaction ACK failed", {
      error: error instanceof Error ? error.message : String(error),
    });
  }
}
...
export async function editOriginalResponse(
  ...
  try {
    ...
    if (!response.ok) {
      const text = await response.text();
      ctx.logger.warn("Edit original response failed", { status: response.status, body: text });
    }
  } catch (error) {
    ctx.logger.error("Edit original response error", {
      error: error instanceof Error ? error.message : String(error),
    });
  }
}
      ...
      switch (payload.op) {
        ...
        case 0: {
          ...
          if (payload.t === "INTERACTION_CREATE") {
            ...
            try {
              // For button clicks (type 3), ACK immediately then process
              if (interaction.type === 3) {
                // Type 6 = DEFERRED_UPDATE_MESSAGE (ACK without updating yet)
                await deferInteraction(ctx, interaction.id, interaction.token, 6);
                const response = await onInteraction(interaction) as { data?: unknown };
                const appId = (interaction as any).application_id as string;
                if (response?.data) {
                  await editOriginalResponse(ctx, appId, interaction.token, response.data);
                }
              } else {
