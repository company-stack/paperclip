...
async function handleButtonClick(
  ...
  if (customId.startsWith("approval_approve_")) {
    ...
    try {
      const apiBase = (ctx as any)._paperclipApiUrl || "http://100.93.113.126:3456";
      await ctx.http.fetch(
        `${apiBase}/api/approvals/${approvalId}/approve`,
        {
          method: "POST",
  ...
  if (customId.startsWith("approval_reject_")) {
    ...
    try {
      const rejectApiBase = (ctx as any)._paperclipApiUrl || "http://100.93.113.126:3456";
      await ctx.http.fetch(
        `${rejectApiBase}/api/approvals/${approvalId}/reject`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ decidedByUserId: `discord:${actor}` }),
